...
| FR-09 | FR | Unit/Integration | EP/BVA | `__tests__/api/api.users-simple.test.js` (add) | P | Add test: invalid order type rejected. |
...
| SR-01 | SR | System | NEG | `__tests__/api/` (auth negative tests) | P | Add explicit no-token checks for protected endpoints. |
| SR-02 | SR | System | NEG | `__tests__/api/` (auth negative tests) | P | Add tampered-token rejection test. |
...
| SR-06 | SR | Integration | Inspection + test | `__tests__/db/db.test.js` (extend) | P | Add explicit check: no plaintext password stored/exposed. |
| RR-01 | RR | System | NEG | `__tests__/api/api.users-generic.test.js` | E | Unknown route returns 404. |
| RR-02 | RR | Integration | NEG | `__tests__/api/` (new) | P | Add malformed ID tests. |
| RR-03 | RR | Integration | NEG | `__tests__/api/` (new) | P | Add missing-resource 404 tests. |
| PR-01 | PR | System | PERF | `__tests__/performance/performance-test.yml` (fix endpoints) | P | Align YAML to actual routes, then measure p95/error rate. |
| PR-02 | PR | System | PERF | `__tests__/performance/performance-test.yml` (fix endpoints) | P | Same for `/login`. |
